...
class resolve_dotted_funcTest(unittest.TestCase):
    ...
    def test_return_func(self):
        ...
        self.assertEqual(func, jinja2_renderer)
...
class parse_templateTest(unittest.TestCase):
    ...
    def test_zipfile(self, mock_urlretrieve):
        ...
        self.assertEqual(set(os.listdir(abs_path[0])), set(["test", ".mrbob.ini"]))
    ...
    def test_zipfile_not_zipfile(self, mock_urlretrieve):
        ...
        self.assertRaises(
            ConfigurationError, self.call_FUT, "http://foobar.com/bla.tar#some/dir"
        )
...
class ConfiguratorTest(unittest.TestCase):
    ...
    def test_parse_questions_basic(self):
        c = self.call_FUT("mrbob.tests:templates/questions1", self.target_dir, {})
    ...
    def test_parse_questions_no_questions(self):
        c = self.call_FUT("mrbob.tests:templates/questions2", self.target_dir, {})
        self.assertEqual(len(c.questions), 0)

    def test_parse_questions_no_questions_section(self):
        self.call_FUT("mrbob.tests:templates/empty2", self.target_dir, {})

    def test_parse_questions_extra_parameter(self):
        c = self.call_FUT("mrbob.tests:templates/questions3", self.target_dir, {})
        self.assertEqual(c.questions[0].extra, {"foobar": "something"})

    def test_parse_questions_all(self):
        c = self.call_FUT("mrbob.tests:templates/questions4", self.target_dir, {})
    ...
    def test_ask_questions_empty(self):
        args = ["mrbob.tests:templates/questions1", self.target_dir, {}]
    ...
    def test_ask_questions_missing(self):
        ...
        c = self.call_FUT(*args)
        c.questions = [
            Question("foo.bar", "fobar?"),
            Question("moo", "Moo?", command_prompt=lambda x: "moo."),
        ]
        c.variables = {"foo.bar": "answer"}
        c.ask_questions()
        self.assertEqual(c.variables, {"foo.bar": "answer", "moo": "moo."})
    ...
    def test_remember_answers(self, mock_render_structure):
        ...
        with open(os.path.join(self.target_dir, ".mrbob.ini")) as f:
            self.assertEqual(f.read().strip(), """[variables]\nfoo.bar = 3""".strip())
    ...
    def test_renderer_default(self):
        ...
        c = self.call_FUT("mrbob.tests:templates/empty", self.target_dir, {})
        self.assertEqual(c.renderer, jinja2_renderer)

    def test_renderer_set(self):
        c = self.call_FUT("mrbob.tests:templates/renderer", self.target_dir, {})
    ...
    def test_ignored_files(self):
        c = self.call_FUT("mrbob.tests:templates/ignored", self.target_dir, {})
    ...
    def test_ignored_directories(self):
        c = self.call_FUT("mrbob.tests:templates/ignored_dirs", self.target_dir, {})
...
class QuestionTest(unittest.TestCase):
    ...
    def test_repr(self):
        ...
        self.assertEqual(
            repr(q),
            six.u("<Question name=foo question='Why?' default=None required=False>"),
        )
    ...
    def test_ask_default_empty(self):
        q = self.call_FUT("foo", "Why?", default="moo", command_prompt=lambda x: "")
        answer = q.ask(DummyConfigurator())
    ...
    def test_ask_default_not_empty(self):
        ...
        q = self.call_FUT("foo", "Why?", default="moo", command_prompt=cmd)
        answer = q.ask(DummyConfigurator())
        self.assertEqual(answer, "foo")
    ...
    def test_ask_no_default_and_required(self):
        ...
        q = self.call_FUT("foo", "Why?", required=True, command_prompt=cmd)
        answer = q.ask(DummyConfigurator())
        self.assertEqual(answer, "foo")
    ...
    def test_ask_help(self):
        ...
        sys.stdout = StringIO()
        q = self.call_FUT("foo", "Why?", help="foobar_help", command_prompt=cmd)
        q.ask(DummyConfigurator())
    ...
    def test_defaults_override(self):
        ...
        c = DummyConfigurator(
            bobconfig={"non_interactive": "True"}, defaults={"foo": "moo"}
        )
        answer = q.ask(c)
        self.assertEqual(answer, "moo")
    ...
    def test_pre_ask_question_multiple(self):
        q = self.call_FUT(
            "foo",
            "Why?",
            pre_ask_question="mrbob.tests.test_configurator:dummy_question_hook mrbob.tests.test_configurator:dummy_question_hook2",
        )
        self.assertEqual(
            q.pre_ask_question, [dummy_question_hook, dummy_question_hook2]
        )
    ...
    def test_pre_ask_question_skipquestion(self):
        q = self.call_FUT(
            "foo",
            "Why?",
            pre_ask_question="mrbob.tests.test_configurator:dummy_question_hook_skipquestion",
        )
        self.assertEqual(q.ask(DummyConfigurator()), None)
    ...
    def test_post_ask_question(self):
        ...
        self.assertEqual(mocked_post_ask_question(), answer)
    ...
    def test_post_ask_question_multiple(self):
        ...
        self.assertEqual(
            q.post_ask_question, [dummy_question_hook, dummy_question_hook2]
        )
    ...
    def test_post_ask_question_validationerror_non_interactive(self):
        ...
        mocked_post_ask_question_validationerror_non_interactive.side_effect = (
            ValidationError
        )
