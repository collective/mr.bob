        ...
        for segment in segments[:-1]:
            ...
            if not isinstance(location, dict):
                raise ConfigurationError(
                    'Cannot assign "%s" to group "%s", subgroup is already used.'
                    % (value, key)
                )
    ...
    for section in ["variables", "defaults", "mr.bob", "questions", "template"]:
        if parser.has_section(section):
            ...
            if section == "questions":
                config[section + "_order"] = [
                    key[:-9] for key, value in items if key.endswith(".question")
                ]
            if section in ["variables", "defaults"]:
                if six.PY3:  # pragma: no cover
                    ...
                    config[section] = dict(
                        [(key, value.decode("utf-8")) for key, value in items]
                    )
            else:
