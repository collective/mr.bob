...
class Configurator(object):
    ...
    def __init__(
        self, template, target_directory, bobconfig=None, variables=None, defaults=None
    ):
        if not bobconfig:
            bobconfig = {}
        if not variables:
        ...
        if self.template_dir in os.path.commonprefix(
            [self.target_directory, self.template_dir]
        ):
            raise ConfigurationError(
        ...
        self.remember_answers = maybe_bool(
            self.bobconfig.get("remember_answers", False)
        )
        ...
        self.post_ask = [
            resolve_dotted_func(f)
            for f in self.templateconfig.get("post_ask", "").split()
        ]
        self.pre_ask = [
            resolve_dotted_func(f)
            for f in self.templateconfig.get("pre_ask", "").split()
        ]
    ...
    def render(self):
        ...
        if self.remember_answers:
            write_config(
                os.path.join(self.target_directory, ".mrbob.ini"),
                "variables",
                self.variables,
            )
        if self.post_render:
...
class Question(object):
    ...
    def __init__(
        ...
        **extra
        ...
        self.help = help
        self.pre_ask_question = [
            resolve_dotted_func(f) for f in pre_ask_question.split()
        ]
        self.post_ask_question = [
            resolve_dotted_func(f) for f in post_ask_question.split()
        ]
        self.extra = extra
    ...
    def __repr__(self):
        return (
            six.u(
                "<Question name=%(name)s question='%(question)s'"
                " default=%(default)s required=%(required)s>"
            )
            % self.__dict__
        )
    ...
    def ask(self, configurator):
        ...
        non_interactive = maybe_bool(
            configurator.bobconfig.get("non_interactive", False)
        )
        if non_interactive:
        ...
        try:
            while correct_answer is None:
                ...
                if six.PY3:  # pragma: no cover
                    ...
                    answer = (
                        self.command_prompt(question.encode("utf-8"))
                        .strip()
                        .decode("utf-8")
                    )
                ...
                # if we don't have an answer or default value and is required, reask
                    ...
                    if non_interactive:
                        raise ConfigurationError(
                            "non-interactive mode: question %s is required but not answered."
                            % self.name
                        )
                    else:
                ...
                for f in self.post_ask_question:
                    try:
                        ...
                        if non_interactive:
                            raise ConfigurationError(
                                "non-interactive mode: question %s failed validation."
                                % self.name
                            )
                        else:
                            correct_answer = None
